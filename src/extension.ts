...
type WebviewToExtensionMessage =
  | { type: 'openApiKeyPageAndSetKey' }
  | { type: 'clearApiKey' }
  | { type: 'fetchProblemFromUrl'; url: string }
  | { type: 'manualPasteProblem'; url: string; problemText: string }
  | { type: 'sendChat'; text: string };

type ExtensionToWebviewMessage =
  | { type: 'authState'; isAuthed: boolean }
  | { type: 'problemLoaded'; url: string; problemText: string; extractedSamples?: { input?: string; output?: string }[] }
  | { type: 'chatMessage'; role: 'user' | 'assistant' | 'system'; text: string }
  | { type: 'clearChat' }
  | { type: 'pasteFallback'; url: string; message: string }
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cpAssistant.open', () => controller.openPanel()),
    vscode.commands.registerCommand('cpAssistant.clearConversation', () => controller.clearConversation()),
    vscode.commands.registerCommand('cpAssistant.setApiKey', () => controller.setApiKeyFlow({ openKeysPage: true })),
    vscode.commands.registerCommand('cpAssistant.clearApiKey', () => controller.clearApiKey()),
    vscode.window.registerWebviewViewProvider('cpAssistant.sidebar', controller, {
...
class CpAssistantController implements vscode.WebviewViewProvider {
  private panel: vscode.WebviewPanel | undefined;
  private view: vscode.WebviewView | undefined;
  private currentProblem: { url: string; text: string; samples: { input?: string; output?: string }[] } | undefined;
  ...
  private attachWebviewHandlers(webview: vscode.Webview) {
  ...
  async setApiKeyFlow(opts: { openKeysPage: boolean }) {
    ...
    const apiKey = await vscode.window.showInputBox({
      ...
      validateInput: (v) => {
        ...
        if (!/^sk-/.test(v.trim())) return 'Expected an OpenAI API key starting with "sk-".';
        return null;
      },
    });
  ...
  private async fetchProblemFromUrl(urlRaw: string) {
  ...
  private async setProblemFromManualPaste(opts: { urlRaw: string; problemText: string }) {
  ...
  private setCurrentProblem(opts: {
    url: string;
    problemText: string;
  ...
  private async sendChat(userText: string) {
    const apiKey = await this.getApiKey();
    if (!apiKey) {
    ...
    const maxTokens = cfg.get<number>('maxTokens', 1200);
...
async function fetchText(url: string): Promise<string> {
  ...
  if (res.status === 403 && isCloudflareChallenge(res, text)) {
    throw new Error(
      'Failed to fetch URL: blocked by Cloudflare bot protection (403). This requires a real browser session; automatic fetch is not supported.',
    );
  }
  ...
  if (
    ...
    throw new Error(
      'Failed to fetch URL: blocked by AWS WAF challenge. Open the URL in a browser and copy/paste the statement, or use another source URL.',
    );
  }
...
function isCloudflareChallenge(res: Response, html: string): boolean {
...
function looksLikeAwsWafChallenge(html: string): boolean {
...
function extractSamples(text: string): { input?: string; output?: string }[] {
  // Best-effort extraction by common headings.
  const lines = text.split('\n').map((l) => l.trimEnd());
  const joined = lines.join('\n');

  const candidates: { input?: string; output?: string }[] = [];
...
async function openaiChatCompletion(opts: {
  ...
  if (!content.trim()) throw new Error('OpenAI response missing message content.');
  return content.trim();
}

function getFirstChatCompletionContent(json: unknown): string {
...
async function safeReadTextFull(res: Response): Promise<string> {
  try {
